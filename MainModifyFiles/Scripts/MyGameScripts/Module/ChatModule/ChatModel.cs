...
//			guildCoolDownTime = remainTime;}
	...
	public string GetUrlInfo(string msg, int itemId,long uniqueId,int type,string uid = "" )     //
	{
		...
		if(index != -1){
			...
			return string.Format("[ffffff]{0}[-][3eff00][url={1},{2},{3},{4}][{5}][/url][-]" ,msg.Substring(0,index),PlayerModel.Instance.GetPlayerId(),type,itemId,uniqueId,msg.Substring(index+3,lastIndex-index-3));
		}
		else{
			return string.Format("[ffffff]{0}[-]" ,msg);
		}
	}
