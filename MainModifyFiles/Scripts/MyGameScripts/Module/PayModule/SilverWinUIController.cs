...
public class SilverWinUIController : WealthBuyWinUIController
{
	private List<ShopItem> _shopItems;

	public SilverWinUIController()
	{
	...
	public void SetData()
	{
		...
		for(int index = 0;index < items.Count;index++)
		{
			if(items[index].shopId == shopId)
			{
				...
				AddWealthCell(icon - 1,"yin" + icon,ItemIconConst.IngotAltas,items[index].virtualCount,ItemIconConst.SilverAltas,items[index].itemCount);
				icon++;
			}
		}
	}
