...
public class MeshEmitterNew : MonoBehaviour
{
    ...
    //float _TimeLastCreate = 0;        //GET WARNING
    bool _Emit = true;
    ...
    ArrayList _Petals = new ArrayList();
    ...
    public bool emit
    {
        ...
        set
        {
            if (value != _Emit)
            {
                ...
                if (_Emit)
                {
                    //_TimeLastCreate = 0;  //GET WARNING
                }
            }
        }
    }

    private Vector3[] vb = null;
    private Vector2[] uvs = null;
    private Color[] clrs = null;
    private int[] ib = null;
    ...
    public void Awake()
    {
        ...
        if (null != _MeshObj)
        {
            ...
            vb = new Vector3[nNumPetals * _vbSrc.Length];
            uvs = new Vector2[nNumPetals * _uvSrc.Length];
            clrs = new Color[nNumPetals * _vbSrc.Length];
            ib = new int[nNumPetals * _ibSrc.Length];
    ...
    void InitOnePetal(Petal pt)
    {
        ...
        pt.pos = pt.spd * r;
        if (!_localSpace) pt.pos = transform.localToWorldMatrix.MultiplyPoint(pt.pos);

        if (!_localSpace) pt.spd = transform.localToWorldMatrix.MultiplyVector(pt.spd);
