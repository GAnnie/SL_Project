	...
	*/
	...
	public static byte[] ToWav(AudioClip clip)
	{
		...
		using(MemoryStream memStream = new MemoryStream(wavData))
		{
			...
			memStream.BeginWrite( emptyByte , 0 , emptyByte.Length , delegate(IAsyncResult ar) {
				memStream.EndWrite( ar );
		} , null );

			// sound data
			Int16[] intData = new Int16[samplesBuf.Length];
			...
			memStream.BeginWrite(bytesData, 0, bytesData.Length , delegate(IAsyncResult ar) {
				memStream.EndWrite( ar );
		} ,null );
			...
			memStream.BeginWrite(riff, 0, 4 , delegate(IAsyncResult ar) {
				memStream.EndWrite( ar );
		},null );
			
			Byte[] chunkSize = BitConverter.GetBytes(memStream.Length - 8);
			memStream.BeginWrite(chunkSize, 0, 4 , delegate(IAsyncResult ar) {
				memStream.EndWrite( ar );
		} , null);
			...
			memStream.BeginWrite(wave, 0, 4 , delegate(IAsyncResult ar) {
				memStream.EndWrite( ar );
		}, null);
			...
			memStream.BeginWrite(fmt, 0, 4 , delegate(IAsyncResult ar) {
				memStream.EndWrite( ar );
		} , null );
			...
			memStream.BeginWrite(subChunk1, 0, 4 , delegate(IAsyncResult ar) {
				memStream.EndWrite( ar );
		} , null );
			...
			memStream.BeginWrite(audioFormat, 0, 2 , delegate(IAsyncResult ar) {
				memStream.EndWrite( ar );
		}, null );
			
			Byte[] numChannels = BitConverter.GetBytes(channels);
			//tempByteArray = MergerArray( tempByteArray , numChannels );
			memStream.BeginWrite(numChannels, 0, 2 , delegate(IAsyncResult ar) {
				memStream.EndWrite( ar );
		} , null);
			
			Byte[] sampleRate = BitConverter.GetBytes(hz);
			//tempByteArray = MergerArray( tempByteArray , sampleRate );
			memStream.BeginWrite(sampleRate, 0, 4 ,delegate(IAsyncResult ar) {
				memStream.EndWrite( ar );
		} , null );
			
			Byte[] byteRate = BitConverter.GetBytes(hz * channels * 2); // sampleRate * bytesPerSample*number of channels, here 44100*2*2
			//tempByteArray = MergerArray( tempByteArray , byteRate );
			memStream.BeginWrite(byteRate, 0, 4  ,delegate(IAsyncResult ar) {
				memStream.EndWrite( ar );
		}, null);
			
			UInt16 blockAlign = (ushort) (channels * 2);
			//tempByteArray = MergerArray( tempByteArray , BitConverter.GetBytes(blockAlign) );
			memStream.BeginWrite(BitConverter.GetBytes(blockAlign), 0, 2 , delegate(IAsyncResult ar) {
				memStream.EndWrite( ar );
		} , null );
			...
			memStream.BeginWrite(bitsPerSample, 0, 2 , delegate(IAsyncResult ar) {
				memStream.EndWrite( ar );
			}, null);
			
			Byte[] datastring = System.Text.Encoding.UTF8.GetBytes("data");
			//tempByteArray = MergerArray( tempByteArray , datastring );
			memStream.BeginWrite(datastring, 0, 4 ,delegate(IAsyncResult ar) {
				memStream.EndWrite( ar );
		}, null );
			
			Byte[] subChunk2 = BitConverter.GetBytes(samples * channels * 2);
			//tempByteArray = MergerArray( tempByteArray , subChunk2 );
			memStream.BeginWrite(subChunk2, 0, 4 ,delegate(IAsyncResult ar) {
				memStream.EndWrite( ar );
		} , null );
...
//			} ,null );
	...
	public static byte[] GetByteUntoWav( AudioClip clip )
	{
		float[] samplesBuf = new float[clip.samples];
		clip.GetData(samplesBuf, 0);
		TipManager.AddTip( samplesBuf.Length.ToString() );
		...
		using(MemoryStream memStream = new MemoryStream(wavData))
		{
			...
			memStream.BeginWrite(tempByte, 0, tempByte.Length , delegate(IAsyncResult ar) {
				memStream.EndWrite( ar );
			} ,null );
		}
