...
public class MissionDataModel {
	...
	*/
	...
	//	任务寻路等待时间
	private readonly float findMissionWaitTime = 1.0f;
	...
	*/
			...
			if (!tSubmitDtoList[i].finish) {
				...
				case SubmitDtoType.HiddenMonster:
					...
					tDebugStr += string.Format("| 暗雷任务： -> 巡逻 {0} 场景 杀掉 {1} 个 {2}\n",
					                           ...
					                           tHiddenMonsterSubmitDto.needCount, tHiddenMonsterSubmitDto.monster.name);
					break;
	...
	private void ApplySubmitProcessing (Mission mission) {
		...
		if (tSubmitDto == null) {
			...
			MainUIViewController.Instance.PopupUseMissionPropsBtn(tApplyItemSubmitDto, () => {
				float tWaitTime = 3.0f;
				CoolDownManager.Instance.SetupCoolDown("_PopupUseMissionProps", tWaitTime, (floatTime) => {
					MainUIViewController.Instance.SetMissionUsePropsProgress(1-floatTime/tWaitTime, true);
				}, () => {
					MainUIViewController.Instance.SetMissionUsePropsProgress(1, false);
	...
	public bool FinishTargetSubmitDto(Mission mission, Npc npc) {
		...
		} else if (submitDtoType == SubmitDtoType.ShowMonster && tSubmitDto.count < tSubmitDto.needCount) {
			//	请求战斗
			BattleShowMonster(mission.id, npc, () => {
				//	TADO
			});
			return true;
		} else if (submitDtoType == SubmitDtoType.Talk || submitDtoType == SubmitDtoType.ApplyItem) {
		...
		if (tSubmitDto.confirm) {
			...
			ProxyWindowModule.OpenConfirmWindow(string.Format("是否提交{0}-{1}？", mission.missionType.name, mission.name),
			                                    ...
			                                    () => {
				DialogueEndToFinishSubmitDto(submitDtoType, mission.id, tSubmitDto.auto, npc);
			});
		} else {
			DialogueEndToFinishSubmitDto(submitDtoType, mission.id, tSubmitDto.auto, npc);
		}
	...
	private void DialogueEndToFinishSubmitDto(SubmitDtoType submitDtoType, int missionID, bool auto, Npc npc) {
		GameDebuger.OrangeDebugLog("调用服务端Finish方法");

		if (submitDtoType == MissionDataModel.SubmitDtoType.Talk) {
			TalkMission(npc.id, () => {
				if (auto) {
	...
	public void WaitTimeFindToMission(Mission mission, bool isExitState) {
		CoolDownManager.Instance.SetupCoolDown("_waitTimeForNextMission", findMissionWaitTime, null, () => {
	...
	private void AcceptMissionFinish(PlayerMissionDto tPlayerMissionDto) {
		GameDebuger.OrangeDebugLog(string.Format("获得新的任务ID:{0} | 是否师门:{1}", tPlayerMissionDto.missionId, tPlayerMissionDto is PlayerFactionMissionDto));
	...
	private void SetAcceMissionQuickFindDialogue(Mission mission) {
		...
		//	延迟生成冥雷怪
		CoolDownManager.Instance.SetupCoolDown("__initShowMonster", 1.0f, null, () => {
			if (tNpc != null) {
	...
	#region	放弃任务
	/// <summary>
	/// 放弃任务(MissionID) -- Drops the mission.
	/// </summary>
	/// <param name="missionID">Mission identifier.</param>
	public void DropMission(int missionID) {
	...
	private bool RemovePlayerMissionDtoByMissionID(int missionID, bool isDropMission = false) {
		...
		PlayerMissionDto tPlayerMissionDto = MissionDataModel.instance.GetPlayerMissionDtoByMissionID(missionID);
		if (tPlayerMissionDto != null) {
			//	注意时序问题，需要先查看是否有下一个任务，才去做删除当前任务操作
	...
	public string GetMissionTitleName(Mission mission, bool isExist, bool isMissionCellTitle) {
		string tStr = string.Format("{0}{1}{2}[-]", isMissionCellTitle?
		                            IsMainMissionType(mission)? _mainColor : _threadColor : "",
		                            //: IsExistCurMission(mission)? _threadColor : "" : "",
		                            mission.missionType.name,
		                            IsMainOrExtension(mission)? string.Format("-{0}", mission.name) : "");
		...
		if (IsFactionMissionType(mission) && isExist) {
			...
			if (tPlayerMissionDto is PlayerFactionMissionDto) {
				...
				tStr = string.Format("{0}{1}({2}/{3})[-]", tStr, _threadColor, tPlayerFactionMissionDto.curRings, "10");
			}
		}
	...
	private string GetCurrentMissionStr(Mission mission, MsgType msgType) {
		...
		string tStr = msgType == MsgType.TargetDesc?
			tCurMissionDialog.goalDesc : msgType == MsgType.Description? tCurMissionDialog.description : "";
		...
		case SubmitDtoType.ApplyItem:
			...
			if (msgType == MsgType.TargetDesc) {
				...
				tStr = Regex.Replace(tStr, "{scene}", string.Format("{0}{1}[-]", _greenColor, tApplyItemSubmitDto.submitNpc.scene == null?
				                                                    string.Format("场景ID错误：{0}", tApplyItemSubmitDto.submitNpc.sceneId)
				                                                    : tApplyItemSubmitDto.submitNpc.scene.name));
		...
		case SubmitDtoType.CollectionPet:
			...
			if (msgType == MsgType.TargetDesc) {
				...
				tStr = Regex.Replace(tStr, "{submitscene}", string.Format("{0}{1}[-]", _greenColor,
				                                                          DataCache.getDtoByCls<SceneMap>(tCollectionPetSubmitDto.submitNpc.sceneId).name));
		...
		case SubmitDtoType.ShowMonster:
			ShowMonsterSubmitDto tShowMonsterSubmitDto = tSubmitDto as ShowMonsterSubmitDto;
			if (msgType == MsgType.TargetDesc) {
				//tStr = Regex.Replace(tStr, "{submitnpc}", string.Format("{0}{1}[-]", _greenColor, tShowMonsterSubmitDto.acceptNpc.name));
			} else if (msgType == MsgType.Description) {
				tStr = Regex.Replace(tStr, "{submitscene}", string.Format("{0}{1}[-]", _greenColor,
				                                                          DataCache.getDtoByCls<SceneMap>(tShowMonsterSubmitDto.submitNpc.sceneId).name));
				//tStr = Regex.Replace(tStr, "{submitnpc}", string.Format("{0}{1}[-]", _blueColor, tShowMonsterSubmitDto.submitNpc.name));
				tStr = Regex.Replace(tStr, "{acceptscene}", string.Format("{0}{1}[-]", _greenColor,
				                                                          DataCache.getDtoByCls<SceneMap>(tShowMonsterSubmitDto.acceptNpc.sceneId).name));
				tStr = Regex.Replace(tStr, "{acceptnpc}", string.Format("{0}{1}[-]", _blueColor, tShowMonsterSubmitDto.acceptNpc.name));
			}
			tStr = Regex.Replace(tStr, "{submitnpc}", string.Format("{0}{1}[-]", _blueColor, tShowMonsterSubmitDto.acceptNpc.name));
			break;
		default:
			tStr = "ERROR:SubmitDtoType.Nothing";
			break;
