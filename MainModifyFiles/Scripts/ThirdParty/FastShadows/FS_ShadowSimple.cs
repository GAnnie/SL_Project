...
public class FS_ShadowSimple : MonoBehaviour {
	...
	public bool limitCornerHeight = true;
	...
	public void CalculateShadowGeometry(){
		...
		if (shadowCaster == null || girth != _girth){
			if (shadowCaster == null){
                ...
                shadowCaster.transform.localPosition = new Vector3(0.0f, 0.03f, 0.0f);//sVector3.zero;
		...
		if (maxProjectionDistance > 0f && Physics.Raycast(r, out rh, maxProjectionDistance, layerMask)){
			...
			if (useLightSource && isPerspectiveProjection){
				...
				for(int i=0;i<4;++i)
				{
					r.origin = cornerGOs[i].transform.position;
					...
					_corners[i] = r.origin + r.direction * rayDist;
					if(_corners[i].y > r.origin.y)
						_corners[i] = new Vector3(_corners[i].x,r.origin.y,_corners[i].z);
				}
				
				if(limitCornerHeight)
				{
					float maxY = Mathf.Max(_corners[0].y,_corners[1].y);
					_corners[0] = new Vector3(_corners[0].x,maxY,_corners[0].z);
					_corners[1] = new Vector3(_corners[1].x,maxY,_corners[1].z);
					
					maxY = Mathf.Min(_corners[2].y,_corners[3].y);
					_corners[2] = new Vector3(_corners[2].x,maxY,_corners[2].z);
					_corners[3] = new Vector3(_corners[3].x,maxY,_corners[3].z);
				}
			}
	...
	void OnDrawGizmos(){
		if (shadowCaster != null){
			...
			if (isGoodPlaneIntersect){
				...
				Gizmos.DrawRay(rh.point,_normal);
